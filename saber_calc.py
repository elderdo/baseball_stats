...
class SaberCalc:
    """
    A sabermetric calculator handling 2026 constants and live API data.
    """
    # 2026 CONSTANTS
    ...
    def fetch_pitcher_stats(self, player_id):
        ...
        try:
            res = requests.get(url).json()
            # Safety check: ensure 'stats' and 'splits' exist and are not empty
            ...
            s = splits[0]['stat']
            # .get(key, 0) handles cases where the API omits fields for zero values
            return {
...
def main():
    ...
    if not args.id or not args.type:
        print("Usage: --id [ID] --type [hitter/pitcher]")
        sys.exit(1)
